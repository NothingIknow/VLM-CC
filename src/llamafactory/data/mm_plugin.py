...
class MMPluginMixin:
    ...
    def _preprocess_image(
        ...
        # 保存float32数据
        float32_data = getattr(image, '_float32_data', None)
        ...
        # 如果原图像有float32数据且尺寸发生了变化，对float32数据进行相应的resize
        if float32_data is not None and (image.width, image.height) != original_size:
            import cv2
            resized_float32 = cv2.resize(float32_data, (image.width, image.height))
            image._float32_data = resized_float32
            # print(f"🔧 BasePlugin resize float32数据: {float32_data.shape} → {resized_float32.shape}")
        elif float32_data is not None:
            # 尺寸没变化，直接保持float32数据
            image._float32_data = float32_data
    ...
    def _regularize_images(self, images: list["ImageInput"], **kwargs) -> dict[str, list["ImageObject"]]:
        ...
        def load_image_preserve_precision(image_input):
            """用cv2读取并返回PIL Image对象，保持16bit精度通过float32数据"""
            # 用cv2读取保持16bit
            img = cv2.imread(image_input, cv2.IMREAD_UNCHANGED)
            
            if img.dtype == np.uint16:
                # 16bit图像: 转换为float32 [0,1] 以保持完整精度
                
                # BGR -> RGB
                if len(img.shape) == 3 and img.shape[2] == 3:
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                
                # 直接归一化到[0,1] (保持16bit的65536级精度)
                img_float = img.astype(np.float32) / 65535.0
                
                # 创建一个特殊的PIL Image对象来携带float32数据
                # 我们将在_get_mm_inputs中直接使用这个float32数据
                pil_img = Image.fromarray((img_float * 255).astype(np.uint8))
                pil_img._float32_data = img_float  # 添加自定义属性保存float32数据
                return pil_img
                
            else:
                # 8bit图像: BGR -> RGB，然后归一化到[0,1]
                if len(img.shape) == 3:
                    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        for image in images:
            if isinstance(image, (str, BinaryIO)):
                image = load_image_preserve_precision(image)
                # image = Image.open(image)

            elif isinstance(image, bytes):
                raise ValueError('导致16bit问题')
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            ...
            mm_inputs.update(image_processor(images, return_tensors="pt", do_rescale=False, **image_processor_kwargs))
    ...
    def _get_mm_inputs(
        ...
        **kwargs,
        ...
        if len(images) != 0:
            ...
            # 检查是否有float32数据，如果有则使用，否则使用PIL图像
            float32_images = []
            all_have_float32 = True
            for img in images:
                if hasattr(img, '_float32_data'):
                    float32_images.append(img._float32_data)
                else:
                    all_have_float32 = False
                    break
            
            # 处理图像：如果所有图像都有float32数据则使用，否则使用PIL图像
            if all_have_float32 and len(float32_images) == len(images):
                # 使用float32数据 (已经归一化到[0,1])
                image_inputs = image_processor(images=float32_images, return_tensors="pt", do_rescale=False, **image_processor_kwargs)
            else:
                # 使用传统PIL图像
...
class Qwen2VLPlugin(BasePlugin):
    ...
    def _preprocess_image(self, image: "ImageObject", **kwargs) -> "ImageObject":
        ...
        # 如果原图像有float32数据，需要对其进行相应的resize处理
        if float32_data is not None:
            import cv2
            # 对float32数据进行相同的resize操作
            resized_float32 = cv2.resize(float32_data, (image.width, image.height))
            image._float32_data = resized_float32
            # print(f"🔧 Resize float32数据: {float32_data.shape} → {resized_float32.shape}")
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            ...
            if float32_images:
                # print('_get_mm_inputs: 使用float32数据')
                # 使用float32数据 (已经归一化到[0,1])
                mm_inputs.update(image_processor(float32_images, return_tensors="pt", do_rescale=False))
            else:
                # 使用传统PIL图像
                # print('_get_mm_inputs: 使用PIL图像')
                mm_inputs.update(image_processor(pil_images, return_tensors="pt", do_rescale=False))
